...
class ResourceFilters extends HTMLElement {
  constructor() {
    ...
    this._handleCategorySelection = this._handleCategorySelection.bind(this);
  }
  ...
  _handleSubmit(event) {
    ...
    const filtersEvent = CustomEvent({
      detail: filters,
      bubbles: true,
      composed: true,
    });
  ...
  _handleCategorySelection(event) {
    ...
    if (clickedButton) {
      const activeSibling = clickedButton.parentElement.querySelector('.active');
